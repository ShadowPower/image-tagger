...
- [x] `P-04` 将源模型资产整理为首版标准 Model Pack。
  ...
  - `pack` 同时支持生成标准目录和供用户安装的 `.itmodel` ZIP。
  ...
  - [x] ModelPackTool 生成的 `.itmodel` 可被同一工具重新验证。
...
- [x] `B-02` 实现 `.itmodel` 安装与卸载。

  依赖：`B-01`。

  工作内容：用 ZipArchive 解压到临时目录，限制总大小/条目数/路径，完整校验后原子移动到托管 Models 目录。

  验收：

  - [x] Zip Slip、绝对路径、重复条目、符号链接和超限压缩包被拒绝。
  - [x] 安装失败不留下半成品目录。
  - [x] 内置包不能卸载；自定义包卸载不删除用户原始 `.itmodel`。
...
- [x] `G-04` 实现设置对话框。
  ...
  工作内容：实现模型安装/校验/卸载入口、主题/行为设置、加速策略、重新检测性能和清除内存缓存。
...
- [ ] `U-08` 对文件输入、Model Pack、日志和第三方依赖进行发布前审计。
  ...
  - [ ] `.itmodel` 路径遍历、压缩炸弹、损坏 hash 和未知 step 测试通过。
...
- [ ] `U-11` 编写与最终实现一致的使用和维护说明。
  ...
  - [ ] README 包含支持平台、安装、打开/识别、标签、元数据、Prompt 和 Model Pack 安装说明。
